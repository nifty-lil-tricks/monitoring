...
t.test('Monitor', (t) => {
  ...
  t.test('should wrap a method', async (t) => {
    ...
    t.equal(recordedParentSpan.spanContext().spanId, parentSpan.spanContext().spanId)
  })
  ...
  t.test('should wrap a nested method', async (t) => {
    ...
    parentSpan.end()
    const [nestedRecordedSpan, recordedSpan, recordedParentSpan] = recordedSpans
  ...
  t.test('should wrap multiple methods', async (t) => {
    ...
    parentSpan.end()
    const [nestedRecordedSpan, recordedSpan1, recordedSpan2, recordedParentSpan] = recordedSpans
    ...
    t.equal(nestedRecordedSpan.parentSpanId, recordedSpan1.spanContext().spanId)
    // Recorded span 1
  ...
  ;[
    ...
    t.test(test, async (t) => {
      ...
      parentSpan.end()
      const [recordedAllowedNestedSpan, recordedAllowedSpan, recordedParentSpan] = recordedSpans

      // Assert
      t.equal(recordedSpans.length, 3)
      t.equal(recordedAllowedNestedSpan.name, 'Service.allowedNested')
      t.strictSame(recordedAllowedNestedSpan.status, { code: SpanStatusCode.OK })
      t.same(recordedAllowedNestedSpan.attributes, {
        'monitoring.class': serviceClassName,
        'monitoring.method': 'allowedNested'
      })
      t.equal(recordedAllowedNestedSpan.parentSpanId, recordedAllowedSpan.spanContext().spanId)
      ...
      t.equal(recordedAllowedSpan.parentSpanId, parentSpan.spanContext().spanId)
      t.equal(recordedParentSpan.spanContext().spanId, parentSpan.spanContext().spanId)
    })
  })
