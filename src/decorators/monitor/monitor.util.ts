import { type Span, diag, trace, context, SpanOptions, Context, SpanStatusCode } from '@opentelemetry/api'

function isPromise<Return>(p: Promise<Return> | Return): p is Promise<Return> {
  return typeof p === 'object' && p !== null && typeof (p as Promise<Return>).then === 'function'
}
...
export class MonitorMethod<Method extends GenericFunction> {
  ...
  #init(inputs: MonitorMethodCallInputs<Method>, activeContext: Context): Span {
    ...
    const span = trace.getTracer(this.#tracer).startSpan(spanName, spanOptions, activeContext)
    span.setAttribute('monitoring.method', this.#metadata.methodName)
    span.setAttribute('monitoring.class', this.#metadata.className)
    return span
  }

  #onResult(inputs: MonitorMethodCallInputs<Method>, span: Span, result: ReturnType<Method>): void {
    debug(`method call successfully completed: '${this.#metadata.methodName}'.`)
    span.setStatus({ code: SpanStatusCode.OK })
  }

  #onError(inputs: MonitorMethodCallInputs<Method>, span: Span, error: unknown): void {
  ...
  #replacementMethod(): Method {
    ...
    function replacementMethod(this: ThisParameterType<Method>, ...args: Parameters<Method>) {
      const inputs: MonitorMethodCallInputs<Method> = { thisArg: this, args }
      const activeContext = context.active()
      const parentSpan = trace.getActiveSpan()
      if (!parentSpan) {
        debug(`no parent span found, not monitoring '${monitorThis.#metadata.methodName}'.`)
      ...
      let result: ReturnType<Method> | Promise<ReturnType<Method>> = undefined as unknown as ReturnType<Method>
      try {
        result = context.with(nestedContext, monitorThis.#method, this, ...args)
        if (isPromise(result)) {
          return result
            .then((data) => {
